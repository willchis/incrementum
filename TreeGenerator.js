...
var TreeGenerator = function (canvas, opts) {
	...
	tg.start = function () {
		...
		if (tg.settings.autoSpawn) {
			self.branch(canvas.WIDTH / 2, canvas.HEIGHT, 0, -3, 10, 30, 0, tg.settings.treeColor);
			intervals.generation = setInterval(function () {
				self.branch((Math.random() * 4) * canvas.WIDTH / 4, canvas.HEIGHT, 0, -Math.random() * 3, 10 * Math.random(), 30, 0, newColor());
	...
	tg.branch = function(x, y, dx, dy, w, growthRate, lifetime, branchColor) {
		...
		if (lifetime > 5 * w + Math.random() * 100 && Math.random() > tg.settings.newBranch) {
			setTimeout(function () {
				...
				self.branch(x, y, 2 * Math.sin(Math.random() + lifetime), 2 * Math.cos(Math.random() + lifetime), (w - lifetime * tg.settings.loss) * tg.settings.branchLoss, growthRate + Math.random() * 100, 0, branchColor);
		...
		if (w - lifetime * tg.settings.loss >= 1) setTimeout(function () {
			self.branch(x, y, dx, dy, w, growthRate, ++lifetime, branchColor);
		}, growthRate);
	}
